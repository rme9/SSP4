...
module.exports.filterResults = function(stores, query) {
    ...
    if(query.age) {
        ...
        var date = max.toDateString();
        max.setDate(max.getDate() - query.age);
        date = max.toDateString();
        stores = stores.filter(function(store){
            for(var i = 0, len = store.items.length; i < len; i++)
            {
                if(store.items[i].name == query.itemName)
                {
                    return store.items[i].reportDate >= max;
                }
            }  
